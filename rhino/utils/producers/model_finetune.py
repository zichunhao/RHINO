...
) -> tuple[ModelWithNewHead, MaskedBatchNorm1d | None]:
    """
    ...
    if train_head:
        ...
        if mode.startswith("train"):
            _load_model_states(
                ...
                model=model,
                head=head,
                part_batch_norm=part_batch_norm,  # Add batch norm to loading
                config=config,
            )

    model = ModelWithNewHead(
        backbone=model, head=head,
    )
    ...
    if weight_path:
        ...
        if part_batch_norm is not None:
            try:
                ...
                raise ValueError(
                    "Batch normalization expected but not found in checkpoint"
                )
        LOGGER.info(
...
def _load_model_with_new_head_states(
    model: ModelWithNewHead,
    part_batch_norm: MaskedBatchNorm1d | None,
    config: dict,
    mode: str = "inference",
) -> None:
    """
    ...
    if load_epoch:
        ...
        LOGGER.info(f"Loading finetuned ModelWithNewHead checkpoint from epoch: {load_epoch}")
        ckpt_path = get_checkpoints_path(config=config, epoch_num=load_epoch)
        ...
        LOGGER.info(f"Loaded finetuned ModelWithNewHead checkpoint from epoch {load_epoch} for dataset: {mode}")
        return
    else:
